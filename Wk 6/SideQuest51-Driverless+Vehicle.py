...
def closest_object(roundTrips):
    ...
    '''roundTrips=input('Enter:')'''
    roundTrips=roundTrips.replace("'",'').split(',')
    roundTrips=[float(i) for i in roundTrips]
    a=roundTrips.index(min(roundTrips))
    b=(min(roundTrips)*344)/2
    if len(roundTrips)==5:
        closestObject = [a,b]
    else:
        closestObject = [-1,-1]
